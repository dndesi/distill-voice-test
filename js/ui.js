...
function renderBrowser(filter = '') {
  ...
  if (analysisFilter === 'none') list = list.filter(s => !_sessionHasAnalysis(s));
  if (exportFilter === 'exported') list = list.filter(s => !!s.exportedAt);
  if (exportFilter === 'not-exported') list = list.filter(s => !s.exportedAt);
  ...
  list.forEach(s => {
    ...
    card.innerHTML = `
      ...
      <div class="sc-body">
        ...
        ${s.exportedAt ? `<span class="sc-status done" title="Exportiert am ${new Date(s.exportedAt).toLocaleDateString('de-DE')}">${icon('check-circle',11,'margin-right:3px;color:var(--green)')} Exportiert</span>` : ''}
      </div>
    `;
...
// v6.76: prüft, ob für eine Sitzung schon irgendeine Analyse gelaufen ist (Standard-Analysen
// oder eigene Prompts) – für den Archiv-Filter "Nur ohne Analyse"
function _sessionHasAnalysis(s) {
  return !!(s.privateAnalysis || s.workAnalysis || s.claudeChapters?.length || s.claudeTopics?.length ||
    s.claude360 || s.claudeSentiment || (s.customResults && Object.keys(s.customResults).length));
}
...
function escHtml(str) {
  // v6.74: String(str ?? '') statt direktem str.replace() – verhindert einen App-weiten
  // Absturz (inkl. Transkript-Anzeige), falls ein Feld mal kein String ist, z.B. wenn ein
  // KI-Anbieter (Mistral) ein Analyse-Feld als Objekt statt als Text zurückliefert.
...
function exportArchPdf() {
  const el = document.getElementById('archView');
  if (!el) return;
  const title = 'Distill Voice – Systemarchitektur v6.98';
...
function renderArchView() {
  ...
  <div style="max-width:960px; margin:0 auto; padding:8px 0 40px">
    ...
    <div style="margin-bottom:24px; display:flex; align-items:flex-start; justify-content:space-between; flex-wrap:wrap; gap:12px">
      <div>
        ...
        <p style="font-size:0.82rem; color:var(--muted); line-height:1.6; margin:0">
          ...
          <span style="color:var(--accent); font-weight:600">Version 6.66</span> · 27 JS-Module
        </p>
      </div>
    ...
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:14px; padding:24px 20px; margin-bottom:24px">
      ...
      <!-- Pfeile -->
      <div style="display:grid; grid-template-columns:repeat(6,1fr); gap:10px; margin-bottom:12px; text-align:center">
        ${archArrow('#fbbf24')}${archArrow('#a78bfa')}${archArrow('#fb923c')}${archArrow('#34d399')}${archArrow('#60a5fa')}${archArrow('#f472b6')}
      </div>
      ...
      <div style="background:linear-gradient(135deg, color-mix(in srgb, var(--accent) 12%, transparent), color-mix(in srgb, var(--accent2) 6%, transparent)); border:2px solid var(--accent); border-radius:12px; padding:18px; margin-bottom:16px; text-align:center">
        ...
        <div style="font-size:0.78rem; color:var(--muted); line-height:1.9">
          ...
          <span style="color:#fb923c; font-weight:500">aiProvider.js</span><br>
    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:10px; margin-bottom:24px">
      ...
      ${flowCard('claude.js', 'KI-Analyse', 'Privat/Arbeit/Gedanken-Analyse, Kapitel, Themen, Stimmung, Anonymisierung, Token-Tracking · v4.81: renderInsights() ruft _refreshAnalysenSubtabs() am Ende – Subtabs aktualisieren sich nach jeder Analyse · v6.26: max_tokens 32000, Freitext-Ergebnisse eigener Prompts über _parseMarkdown() mit Anker-Links (_jumpToAnchor()) · v6.62: Modell-Historie – _archiveAnalysisRun()/_renderRunPills(), erneute Analyse mit anderem Anbieter überschreibt das Ergebnis nicht mehr, Pillen-Switcher pro Analyse-Block · v6.67: MD-/TXT-Export eigener Prompts unterstützt jetzt alle 11 Ausgabe-Feld-Typen (vorher fehlten boolean, rating, quote, key_value, list_with_date, tag_list) · v6.68: Tabellen-Fallback ohne s.columns (Kopf-/Trennzeile), _mdInline() gegen Zeilenumbrüche in Listen/Zitaten/Tabellenzellen · v6.70: Sitzungsname fehlte in _mdFilename() (Dateiname), exportCustomResultPdf() und exportCustomResultText() (Titel/Inhalt) – jetzt ergänzt · v6.74: _claudeFetchWithRetry() (und aiProvider.js: _mistralFetchWithRetry()/callOllamaAPI()) mit 120s-Timeout über _fetchWithTimeout() (drive.js) – verhindert endlos hängende Analysen · renderInsights() + analysePrivate()/analyseWork() prüfen Textfelder (summary/dynamics/zwischenzeilen) jetzt per typeof statt nur auf leer, damit ein abweichendes KI-Antwortformat (z.B. Mistral) nicht mehr die Sitzungsansicht zum Absturz bringt · v6.75: extractJSON() maskiert rohe Steuerzeichen (z.B. unescapte Zeilenumbrüche) innerhalb von JSON-Strings selbst nach, bevor JSON.parse() läuft – behebt "Expected \',\' or \'}\'..."-Abbrüche bei Kapitel-/Themen-/eigenen Prompt-Analysen · v6.77: TIMEOUT_MS (hier + aiProvider.js) 120s→300s – lange Sitzungen (z.B. 72min) brauchten legitim länger als 120s und liefen in den v6.74-Timeout, kein echter Hänger · v6.78: exportAnalysisMd() in _prepareAnalysisMd() + Download aufgeteilt, neue shareAnalysisMd() – Analyse als MD-Datei direkt an den OS-Teilen-Dialog (navigator.share), Fallback auf Download ohne Datei-Teilen-Unterstützung · v6.79: _renderRunPills() zeigt die KI-Anbieter-Pille jetzt auch bei nur einer Analyse (vorher erst ab 2 archivierten Läufen) · v6.84: Teilen-Button (Web Share API) wieder entfernt – auf macOS+Chrome verweigert navigator.share() Datei-Freigaben zuverlässig mit "Permission denied", Fallback war ohnehin derselbe Download wie der MD-Button; neue saveAnalysisMdAs() nutzt showSaveFilePicker() (File System Access API), damit die MD-Datei in einen frei wählbaren Ordner statt immer in den Downloads-Ordner gespeichert wird · v6.85: _buildMdFrontmatter() um 4 optionale Felder für den Obsidian-Ingest ergänzt (quelle_link, quelle_typ, serie, teil) – manuell im Transkript-Header pflegbar (updateSessionQuelle()), rein additiv, keine automatische Ableitung aus session.type · v6.86: MD-Speichern/-Export prüft jetzt benannte Sprecher (_guardSpeakersNamed(), gleiche Logik wie beim Analyse-Start), teilnehmer-Liste im Frontmatter filtert bewusstes "kein zweiter Sprecher" (_isNoSecondSpeaker()) raus statt es als Namen zu zeigen · v6.87: checkSpeakersNamed() erkennt jetzt auch Platzhalter-Namen wie "Sprecher A" (_isUnclearSpeakerName()) statt nur leere Felder – betrifft dadurch auch die Analyse-Start-Guards, die dieselbe Funktion nutzen · v6.90: _buildMdFrontmatter() um fünftes optionales Feld "bereich" ergänzt, 1:1 aus session.type abgeleitet (bereichMap, kein neues UI-Feld) · v6.92: _markSessionExported() setzt session.exportedAt bei erfolgreichem Analyse-MD-Export/-Speichern (exportAnalysisMd()/saveAnalysisMdAs()) – Grundlage für Export-Badge/-Filter im Sitzungs-Archiv · v6.93: Bugfix – Bearbeiten-Icon bei Analyse-Feldern/-Einträgen war unsichtbar (icon(\'pencil\') nicht in icons.js registriert), auf icon(\'edit-2\') umgestellt', '#a78bfa')}
      ...
      ${flowCard('app.js', 'Initialisierung', 'async init() → await initStorage() → IndexedDB laden vor UI-Start · Theme-Toggle · Upload-Schrittvalidierung · Drag & Drop · v6.94: checkPendingShares() zusätzlich bei visibilitychange/pageshow (nicht nur einmalig in init()), da Android beim Teilen ein bereits offenes Fenster nur in den Vordergrund holen kann, ohne dass init() erneut läuft · v6.97: Fix-Versuch – manifest.json (baut den nativen Android-Teilen-Eintrag) hatte als einzige zentrale Datei keinen Cache-Buster, jetzt manifest.json?v=X.XX wie alle anderen Dateien · v6.98: Teilen-Popup-Problem bleibt trotzdem ungelöst (vermutlich außerhalb der App, Androids WebAPK-Rebuild), Suche eingestellt – Debug-Panel entfernt, visibilitychange/pageshow-Trigger aus v6.94 bleiben bestehen', '#c084fc')}
