...
- MD-Speichern/-Export (MD-Button, "Speichern unter…", Transkript-MD) prüft jetzt vorher, ob Sprecher A/B benannt sind – dieselbe Pflichtpfad-Guard-Logik, die schon länger den Analyse-Start absichert (`runSingleAnalysis()`/`openAnalyseModal()`, dort unverändert gelassen). Neue Funktion `_guardSpeakersNamed(session)` (`js/claude.js`) markiert fehlende Felder rot, scrollt dorthin und zeigt einen Hinweis-Toast, statt eine MD-Datei mit `<unbekannt>` zu erzeugen. Aufgerufen in `_prepareAnalysisMd()` (deckt `exportAnalysisMd()` sowie den Download-Fallback von `saveAnalysisMdAs()` ab), `exportTranscriptMd()` und zusätzlich vor dem Ordner-Auswahldialog in `saveAnalysisMdAs()` (damit der Nutzer nicht erst einen Speicherort wählen muss, bevor der Hinweis kommt). TXT/PDF-Export bewusst unverändert (nicht angefragt).
- Frontmatter-`teilnehmer`-Liste bereinigt: Ist bei einem Sprecher bewusst "keinen"/"niemand"/"0" o.ä. eingetragen (erkannt über den bereits bestehenden Helfer `_isNoSecondSpeaker()`, seit v6.55), taucht dieser Platzhalter jetzt nicht mehr als Name in `teilnehmer` auf – nur noch der eine echte Sprecher. `_buildMdFrontmatter()`: Filter von `.filter(Boolean)` auf `.filter(name => name && !_isNoSecondSpeaker(name))` erweitert.

## v6.85 (Stand: 06.09.2026)
- Feature: Frontmatter um 4 optionale Felder für den Obsidian-Ingest erweitert – `quelle_link` (z.B. YouTube-URL), `quelle_typ` (Enum: `gespraech`/`meeting`/`webinar`/`tutorial`/`reflexion`), `serie` (Name der Reihe) und `teil` (Nummer). Neuer Abschnitt "Quelle & Serie" im Transkript-Header (`index.html`, unter "Beteiligte Personen"): Text-Feld für den Link, Dropdown für den Typ (deutsche Labels, `value` = die exakten kleingeschriebenen Enum-Strings ohne Umlaut – wichtig für den Ingest-Workflow), zwei weitere Felder für Serie/Teil. Neue Funktion `updateSessionQuelle()` (`js/claude.js`, analog `updateSessionPersons()`) speichert alle 4 Werte in `session.quelleLink`/`quelleTyp`/`serie`/`teil` – ein leerer Wert wird per `delete` aus dem Session-Objekt entfernt statt als leerer String gespeichert. `_buildMdFrontmatter()` schreibt die 4 Felder nur, wenn gesetzt, direkt nach dem bestehenden `perspektive`-Feld, vor dem schließenden `---` – rein additiv, bestehende Feldnamen/Reihenfolge/Verhalten bei fehlenden Werten unverändert. Bewusst **keine** automatische Ableitung von `quelle_typ` aus `session.type` (abgestimmt mit Daniel) – die Kategorien sind nicht deckungsgleich (`session.type` kennt privat/arbeit/wissen/gedanken, nicht meeting/webinar), eine geratene Zuordnung wäre potenziell falsch. Git-Workflow für diese Änderung: direkt auf `main` gepusht (wie alle v6.x-Änderungen dieser Session), abweichend von `DECISIONS.md` (Feature-Branch+PR) – mit Daniel abgestimmt, `DECISIONS.md` gilt hier als veraltet (Stand v4.6).

## v6.84 (Stand: 05.09.2026)
- Teilen-Button (Web Share API) wieder entfernt: auf macOS + Chrome fiel er ohnehin auf denselben Download zurück wie der MD-Button daneben (v6.83-Fallback bei `NotAllowedError`) – zwei Buttons für dieselbe Funktion, kein Mehrwert. Statt der browser-/OS-seitig unzuverlässigen Freigabe komplett entfernt (`shareAnalysisMd()` gelöscht, `_prepareAnalysisMd()`/`_getKernbefund()` auf ihre ursprüngliche Signatur ohne `skipEnrichment`/`skipMiniCall` zurückgebaut).
- Neu: "Speichern unter…"-Button (gleiche Position wie vorher der Teilen-Button, Disketten-Icon) speichert die MD-Datei über die File System Access API (`window.showSaveFilePicker()`) direkt in einen selbst gewählten Ordner – der eigentliche Wunsch, den der Teilen-Button nie erfüllt hat (der landete immer im Downloads-Ordner). Neue Funktion `saveAnalysisMdAs()` (`js/claude.js`): öffnet zuerst den Auswahldialog (braucht die frische Nutzer-Geste), baut danach den MD-Inhalt auf und schreibt ihn über den zurückgegebenen `FileSystemFileHandle`. Unterstützt der Browser die API nicht (Firefox, Safari), fällt der Button automatisch auf den gewohnten Download zurück, mit Info-Toast. Neue Hilfsfunktion `_analysisPerspektive(type, session)` aus `_prepareAnalysisMd()` herausgezogen, da `saveAnalysisMdAs()` den Dateinamen für den Auswahldialog schon kennen muss, bevor der eigentliche MD-Inhalt (inkl. evtl. Tag-Generierung) gebaut wird.

## v6.83 (Stand: 05.09.2026)
- Fix: Teilen-Button brach auch nach dem Timing-Fix aus v6.82 auf manchen Systemen weiterhin mit `Permission denied` ab (bestätigt mit macOS + Chrome). Ursache diesmal browser-/betriebssystemseitig statt im Timing: `navigator.canShare({files:[...]})` meldet Unterstützung für die Datei-Freigabe, der eigentliche macOS-Freigabe-Dialog von Chrome lehnt die `.md`-Datei dann aber trotzdem mit `NotAllowedError`/"Permission denied" ab – ein bekanntes, von Anwendungscode aus nicht behebbares Verhalten (kein Bug in `shareAnalysisMd()`/`_prepareAnalysisMd()` selbst). Fix: `shareAnalysisMd()` (`js/claude.js`) fängt im `catch`-Block gezielt `NotAllowedError` ab und fällt automatisch auf denselben Download zurück, der ohnehin schon greift, wenn ein Browser `canShare()` mit `false` beantwortet – inkl. kurzem Info-Toast statt Fehlermeldung. `AbortError` (Nutzer bricht den OS-Dialog selbst ab) bleibt weiterhin bewusst ohne Meldung. Ergebnis: der Teilen-Klick endet nie mehr in einem sichtbaren Fehler – er öffnet entweder den OS-Freigabe-Dialog erfolgreich oder lädt die MD-Datei direkt herunter.

## v6.82 (Stand: 05.09.2026)
- Fix: Teilen-Button schlug trotz sichtbarem Icon (v6.81) weiterhin mit `Permission denied` fehl. Ursache: `shareAnalysisMd()` (`js/claude.js`) ruft `_prepareAnalysisMd()` auf, die bei fehlenden Tags automatisch welche über einen echten KI-API-Call nachlädt (`_generateTagsMiniCall()`, seit v6.20) und bei fehlendem Kernbefund-Feld zusätzlich `_kernbefundMiniCall()` nachschieben kann. `navigator.share()` mit Dateien braucht eine noch aktive Nutzer-Geste – der Netzwerk-Roundtrip dazwischen dauert lange genug, dass der Browser den ursprünglichen Klick als abgelaufen wertet und die Freigabe mit `NotAllowedError`/"Permission denied" verweigert. Fix: `_prepareAnalysisMd(type, skipEnrichment=false)` und `_getKernbefund(type, session, skipMiniCall=false)` bekommen einen neuen Parameter, der diese zwei KI-Anreicherungsschritte überspringt – nur `shareAnalysisMd()` ruft `_prepareAnalysisMd(type, true)`, damit zwischen Klick und `navigator.share()` ausschließlich synchrone String-Verarbeitung liegt (keine Promise-Kette mit echtem I/O, die die Nutzer-Geste verbraucht). `exportAnalysisMd()` (Download-Button) bleibt unverändert mit voller Anreicherung, da ein Download keine Nutzer-Geste über die Zeit retten muss. Kompromiss: eine geteilte MD-Datei kann dadurch ohne Auto-Tags bzw. mit `<unbekannt>` als Kernbefund ankommen statt komplett zu scheitern.

## v6.81 (Stand: 05.09.2026)
- Fix: Teilen-Button (und TXT/PDF-Buttons) bei eigenen Prompts ohne Icon – bei "Teilen" besonders auffällig, da dieser Button (anders als TXT/PDF) keine Textbeschriftung hat und dadurch praktisch unsichtbar/unklickbar wirkte. Ursache: die Export-Leiste eigener Prompts (`js/claude.js`, ~Zeile 1387-1393) forderte die Icons `copy`/`printer`/`share-2` über `icon()` an – diese Funktion (`icons.js`) kennt nur eine feste Liste von 62 Icons, alle drei fehlen darin, `icon()` liefert dann kommentarlos `''`. Fix: auf `iconLucide()` umgestellt (volles Lucide-Set), exakt das gleiche Muster, das im selben Block bereits für den Titel-Icon (`icoName`) verwendet wird – der dortige `lucide.createIcons({nodes:[customContainer]})`-Aufruf deckt damit automatisch auch die neuen Icons ab. Die 6 fest eingebauten Analysen (`index.html`, nutzen bereits `data-lucide`/`iconLucide()`) waren nie betroffen.

## v6.80 (Stand: 31.08.2026)
- Fix: Löschen eines einzelnen Eintrags innerhalb einer Analyse (`deleteAnalysisItem()`, `js/sessions.js`) sprang danach immer zum ersten Analyse-Tab der Sitzung statt beim gerade offenen Tab zu bleiben – der Nutzer musste manuell zurückschalten. Ursache: `_refreshAnalysenSubtabs()` (`js/ui.js`) läuft nach jedem `renderInsights(s)`-Aufruf (u.a. aus `deleteAnalysisItem()`, `deleteAnalysis()`, `deleteCustomAnalysis()`) und baute die Tab-Leiste komplett neu auf – am Ende stand dort hart `switchAnalysenSubtab(visible[0].id)`, unabhängig davon, welcher Tab vorher aktiv war. Fix: Die Funktion merkt sich jetzt vor dem Neuaufbau den aktiven Tab (`container.querySelector('.sdc-subtab-active')?.dataset.block`) und wählt als Ziel-Tab (`targetId`) diesen vorherigen Tab, sofern er nach dem Neuaufbau noch in `visible` enthalten ist (d.h. die Analyse noch Inhalt hat) – sonst (z.B. weil genau diese Analyse gerade komplett gelöscht wurde, `deleteAnalysis()`/`deleteCustomAnalysis()`) weiterhin Fallback auf den ersten sichtbaren Tab, wie bisher. Verhalten beim allerersten Öffnen einer Sitzung (noch kein aktiver Tab vorhanden) unverändert. Betrifft alle Aufrufer von `_refreshAnalysenSubtabs()` gleichermaßen (u.a. auch Tab-Wechsel auf "Analysen", 360°-Analyse in `features.js`).

## v6.79 (Stand: 31.08.2026)
- Fix/UX: Die KI-Anbieter-Pille ("Claude"/"Mistral") über einer Analyse war nur sichtbar, wenn dieselbe Analyse bereits mit einem anderen Anbieter erneut gelaufen war – der Pillen-Switcher aus v6.62 (`_renderRunPills()`, `js/claude.js`) brach bisher komplett ab (`if (!runs?.length || !meta) return '';`), solange keine archivierten Alternativ-Läufe (`session[key+'Runs']`) existierten. Da das der absolute Regelfall ist (eine Analyse wird meist nur einmal ausgeführt), war der tatsächlich verwendete Anbieter – obwohl längst in `session[key+'Meta']` gespeichert – in der Praxis fast nie sichtbar. Fix: Bedingung auf `if (!meta) return '';` reduziert – die "Aktueller Stand"-Pille erscheint jetzt immer, sobald überhaupt eine KI-Analyse gelaufen ist, weiterhin als erste Zeile des jeweiligen Analyse-Blocks (unverändert). Bei vorhandenen archivierten Läufen bleibt sie wie bisher anklickbar zum Umschalten, bei nur einem Lauf ist sie rein informativ (kein `onclick`-Handler an den zusätzlichen Pillen, da `runs` dann leer ist). Gleiche Anpassung am inline-duplizierten Pillen-Code für eigene Prompts (`customResultsRuns`/`customResultsMeta`, ebenfalls `js/claude.js`, ~Zeile 1360). Betroffen: alle 6 eingebauten Analysen (nutzen gemeinsam `_renderRunPills()`) + eigene Prompts.

## v6.78 (Stand: 31.08.2026)
- Feature: Teilen-Button bei den Analyse-Ergebnissen. Auslöser: das native "Teilen" des Browsers (Browser-Chrome, nicht die App) teilt auf einer clientseitig gerenderten SPA ohne View-Routing nur die generische App-URL – nicht hilfreich, wirkt kaputt. Statt Browser-Teilen zu reparieren (nicht möglich, App-seitig nicht beeinflussbar) neuer eigener Teilen-Button in der App: bei allen 6 eingebauten Analysen (Gesprächs-/Arbeitsanalyse, Stimmung, Kapitel, Themen, 360°) sowie bei eigenen Prompts (`customResults`), direkt neben dem bestehenden "MD"-Export-Button in der jeweiligen Export-Leiste. Mit Daniel abgestimmt (2 Rückfragen): Inhalt = MD-Datei (nicht Text/Link), Umfang zuerst nur die Analyse-Export-Leiste (nicht Sitzungsseite/Notizen/Transkript – das wäre ein separater Schritt). Technisch: `exportAnalysisMd()` (`js/claude.js`) in eine gemeinsame `_prepareAnalysisMd(type)` (baut MD-Inhalt + Dateiname, identische Logik wie bisher) und den bestehenden Download-Teil aufgeteilt, neue `shareAnalysisMd(type)` nutzt dieselbe `_prepareAnalysisMd()` und übergibt das Ergebnis als `File`-Objekt an `navigator.share({files:[...]})` (Web Share API Level 2) – öffnet den OS-Teilen-Dialog (WhatsApp, Mail, Notizen, AirDrop, …) direkt mit der MD-Datei, ohne Umweg über "Herunterladen → manuell anhängen". Unterstützt der Browser kein Datei-Teilen (geprüft über `navigator.canShare({files:[...]})`, z.B. Desktop-Firefox), fällt der Button automatisch auf denselben Download wie der MD-Button zurück, mit kurzem erklärendem Toast statt eines wirkungslosen Klicks. Bricht der Nutzer den OS-Teilen-Dialog ab (`AbortError`), erscheint bewusst keine Fehlermeldung. UI: neuer Icon-Button (Lucide `share-2`, wie bereits an anderer Stelle in `index.html` verwendet) in `index.html` (6× statisch) und im dynamisch gerenderten Custom-Prompt-Block in `js/claude.js` (nutzt den dortigen `icon()`-Helfer, `lucide.createIcons()` läuft dort bereits nach dem Rendern). Bewusst nicht umgesetzt (erste Iteration, mit Daniel abgestimmt): Teilen-Button auf der Sitzungsseite, bei Notizen oder beim Transkript – folgt bei Bedarf als eigener Schritt, sobald sich das MD-Vorgehen hier bewährt hat.
